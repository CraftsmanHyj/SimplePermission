...
internal object PermissionApi {
     ...
     * 判断某个权限集合是否包含特殊权限
     */
    fun containsSpecialPermission(permissions: Array<String>): Boolean {
        if (permissions.isEmpty()) return false
        for (permission in permissions) {
            if (isSpecialPermission(permission)) return true
        }
        return false
    }
    ...
    fun isSpecialPermission(permission: String): Boolean {
        return Permission.MANAGE_EXTERNAL_STORAGE == permission
                ...
                || Permission.PACKAGE_USAGE_STATS == permission
                || Permission.SCHEDULE_EXACT_ALARM == permission
                || Permission.BIND_NOTIFICATION_LISTENER_SERVICE == permission
                || Permission.ACCESS_NOTIFICATION_POLICY == permission
                || Permission.REQUEST_IGNORE_BATTERY_OPTIMIZATIONS == permission
    }
